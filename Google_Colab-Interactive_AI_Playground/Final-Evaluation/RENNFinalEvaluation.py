...
def initializePackages(devicePackage, ioPackage, pdPackage, paPackage, pqPackage, zstdPackage, levenshteinPackage, cmaPackage, chosenDataSetPackage, seedToSet="", useBitLinear=False):
    global device, useBitNet, io, pd, pa, pq, zstd, levenshtein, cma, chosenDataSet, seed
...
def forward_hook(module, input, output):
    global layer, source, dictionaryForSourceLayerNeuron, dictionaryForLayerNeuronSource, \
        ...
        sourceArray, hidden_sizes, fileName, layersToCheck
...
def attachHooks(hookLoader, model, filename = "", sourceOffset=0):
...
def runHooks(train_dataloader, model, layersParameter=layers, context_length=1, layersToCheckParameter=[]):
    global layers, dictionaryForSourceLayerNeuron, dictionaryForLayerNeuronSource, activationsBySources, activationsByLayers, \
        ...
        mtDictionaryForSourceLayerNeuron, mtDictionaryForLayerNeuronSource, mtActivationsBySources, mtActivationsByLayers, contextLength, layersToCheck
    ...
    attachHooks(train_dataloader, model, filename="Training", sourceOffset=0)
...
def initializeEvaluationHook(hidden_sizes, eval_dataloader, eval_samples, model, filename="Evaluation", sourceOffset=0):
    ...
    with torch.no_grad():
        ...
        attachHooks(eval_dataloader, model, filename, sourceOffset)
...
def identifyClosestSourcesByMetricCombination(name, closestSources, metricsOutputs, metrics_indices, metric_weights=METRIC_WEIGHTS, mode=""):
    global layers
...
def getMostUsedSourcesByName(name, sources, closestSources, mode):
    sourceCounter, mostUsed, differences, mostUsedSourcesPerLayer, differencesPerLayer = getMostUsedByName(sources, mode)
...
def getMostUsedByName(sources, mode=""):
    mostUsed = []
    mostUsedSourcesPerLayer = []
    differences = []
    differencesPerLayer = []
